...
def test_decide_direction_holds_when_edge_is_weak():
    cfg = load_config()
    # High confidence on one side but tiny buy/sell gap after hold mass.
...
def test_decide_direction_takes_strong_buy_aligned_with_return():
    cfg = load_config()
...
def test_decide_direction_rejects_return_disagreement():
    cfg = load_config()
    probs = np.array([0.15, 0.20, 0.65], dtype=np.float64)
